...
export default function Profile() {
    const [disabled, setDisabled] = useState(true); // to toggle between edit and save mode
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-8 gap-2 mt-8 auto-rows-[150px]">
            <div className="bg-white col-span-1 sm:col-span-2 lg:col-span-3 row-span-3 rounded-2xl shadow-xl dark:bg-primary-dark">  {/* first cell */}
                ...
                <p className="text-prof-text-secondary m-4 dark:text-text-secondary-dark">
                    ...
                    <span className="font-bold dark:text-text-dark blo">Member since:</span> {formatted}
                </p>
                <p className="text-prof-text-secondary m-4 dark:text-text-secondary-dark">
                    <i className="fa-solid fa-chart-line dark:text-text-dark"></i>
                    <span className="font-bold dark:text-text-dark">Fitness Level:</span> {user?.activityLevel}
                </p>
            </div>
            <div className="bg-white col-span-1 sm:col-span-2 lg:col-span-5 row-span-3 rounded-2xl shadow-xl dark:bg-primary-dark"> {/* second cell */}
                ...
                <div className="grid grid-cols-2 gap-2 m-4 auto-rows-[80px]">
                    <div>
                        ...
                        <ProfileInput disabled={disabled} type="text" content={user?.fullName ?? ''} profile={profile} setProfile={setProfile} field="name" />  {/*full name */}
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Age</h4>
                        <ProfileInput disabled={disabled} type="number" content={user?.age?? ""} profile={profile} setProfile={setProfile} field="age" /> {/*age */}   
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Gender</h4>
                        <ProfileInput disabled={disabled} type="select" content={user?.gender ?? ""} profile={profile} setProfile={setProfile} field="gender" /> {/*gender */}
                    </div>
                    <div className="col-span-2">
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Bio</h4>
                        <ProfileInput disabled={disabled} type="bio" content={user?.bio ?? ""} profile={profile} setProfile={setProfile} field="bio" /> {/*bio */}
                    </div>
                </div>
            </div>
            <div className="bg-white col-span-1 sm:col-span-2 lg:col-span-4 row-span-3 rounded-2xl shadow-xl xl dark:bg-primary-dark">  {/* third cell */}
                ...
                <div className="grid grid-cols-2 gap-2 m-4 auto-rows-[80px]">
                    <div>
                        ...
                        <ProfileInput disabled={disabled} type="number" content={user?.height ?? ''} profile={profile} setProfile={setProfile} field="height" /> {/*height */}
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Weight (kg)</h4>
                        <ProfileInput disabled={disabled} type="number" content={user?.currentWeight ?? ''} profile={profile} setProfile={setProfile} field="weight" /> {/*weight */}
                    </div>
                    <div className="col-span-2">
            ...
            <div className="bg-white col-span-1 sm:col-span-2 lg:col-span-4 row-span-3 rounded-2xl shadow-xl xl dark:bg-primary-dark"> {/* fourth cell */}
                ...
                <div className="grid grid-cols-1 gap-2 m-4 auto-rows-[80px]">
                    <div>
                        ...
                        <ProfileInput disabled={disabled} type="select" content={user?.primaryGoal ?? ''} profile={profile} setProfile={setProfile} field="primaryGoals" /> {/*primary goals */}
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Target Weight (kg)</h4>
                        <ProfileInput disabled={disabled} type="number" content={user?.targetWeight ?? ''} profile={profile} setProfile={setProfile} field="targetWeight" /> {/*target weight */}
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Activity Level</h4>
                        <ProfileInput disabled={disabled} type="select" content={user?.activityLevel ?? ''} profile={profile} setProfile={setProfile} field="activityLevel" /> {/*activity level */}
                    </div>
                    <div>
                        <h4 className="m-4 mb-2 text-prof-text-secondary font-bold dark:text-text-dark">Progress to go</h4>
                        <div className="bg-green-200 font-bold rounded px-3 py-2 w-full text-prof-text-secondary dark:text-prof-text">{Math.abs(Number(user?.targetWeight ?? 0) - Number(user?.currentWeight ?? 0))} kg</div> {/*progress to go */}
                    </div>
                </div>
            </div>
        </div>
